#!/usr/bin/env node
/**
 * One-shot release: version bump -> build -> zip -> git tag.
 *
 * Usage:
...
if (!bumpArg) {
  console.error("Usage: node scripts/release.mjs <patch|minor|major|X.Y.Z> [--no-tag] [--no-zip] [--no-commit]");
  process.exit(1);
}
...
// --- 1. clean working tree check (skip entirely if both git steps disabled) ---
...
if (!skipGit) {
  try {
    ...
    console.error("Not a git repo (or git unavailable). Re-run with --no-commit --no-tag to skip git ops.");
    process.exit(1);
  }
}
...
if (!flags.has("--no-zip")) {
  ...
  const zipPath = resolve(releasesDir, `sincerity-tools-v${newVersion}.zip`);
